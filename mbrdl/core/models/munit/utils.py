...
from data.transforms import STATS
...
def __write_images(image_outputs, display_image_num, file_name, denorm=None):
...
def write_2images(image_outputs, display_image_num, image_directory, postfix, dataset):
    n = len(image_outputs)
    # denorm = K.Denormalize(*STATS[dataset])
    # __write_images(image_outputs[0:n//2], display_image_num, '%s/gen_a2b_%s.jpg' % (image_directory, postfix), denorm)
    # __write_images(image_outputs[n//2:n], display_image_num, '%s/gen_b2a_%s.jpg' % (image_directory, postfix), denorm)
